...
function mapWalletRequest(r) {
  return {
    id: 'wr-' + r.id,
    type: r.type === 'add' ? 'deposit' : 'withdraw',
    amount: r.amount,
    title: (r.type === 'add' ? 'Add Money' : 'Withdraw') + (r.method ? ` • ${r.method}` : ''),
    subtitle: r.status === 'pending' ? 'Verification pending' : r.status === 'rejected' ? 'Request rejected' : 'Verified by admin',
    status: r.status,
    createdAtMs: r.requestedAt?.toMillis?.() || 0,
    createdAtLabel: formatDate(r.requestedAt),
  }
}

function mapTransaction(t) {
  const def = TRANSACTION_TYPES[t.type] || {}
  return {
    id: 'tx-' + t.id,
    type: t.type,
    amount: t.amount,
    title: t.title || def.label || t.type,
...
export default function TransactionHistory() {
  ...
  const allItems = useMemo(() => {
    const merged = [...walletRequests.map(mapWalletRequest), ...transactions.map(mapTransaction)]
    merged.sort((a, b) => b.createdAtMs - a.createdAtMs)
    return merged
  }, [walletRequests, transactions])
  ...
  return (
    <div className="screen page-fade-in" style={{ paddingBottom: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
        ...
        <h2 style={{ margin: 0, fontSize: 20, color: '#fff' }}>Transaction History</h2>
      </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 18 }}>
        <div style={{ flex: 1, background: 'rgba(16,60,45,0.35)', border: '1px solid rgba(52,224,161,0.25)', borderRadius: 14, padding: '14px 16px' }}>
          <div style={{ fontSize: 11, letterSpacing: 0.5, color: '#34e0a1', marginBottom: 6 }}>TOTAL IN</div>
        ...
        <div style={{ flex: 1, background: 'rgba(60,20,20,0.35)', border: '1px solid rgba(248,113,113,0.25)', borderRadius: 14, padding: '14px 16px' }}>
          <div style={{ fontSize: 11, letterSpacing: 0.5, color: '#f87171', marginBottom: 6 }}>TOTAL OUT</div>
            ...
            >
              ...
              <div style={{ textAlign: 'right', flexShrink: 0 }}>
                ...
                <div style={{ fontSize: 10.5, letterSpacing: 0.5, color: item.status === 'pending' ? '#fbbf24' : item.status === 'rejected' ? '#f87171' : '#6b7280', marginTop: 2 }}>
                  {item.status === 'pending' ? 'PENDING' : item.status === 'rejected' ? 'REJECTED' : 'TOTAL'}
                </div>
              </div>
            </div>
          )
