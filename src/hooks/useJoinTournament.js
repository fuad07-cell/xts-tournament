  ...
  async function joinTournament(t, details = null) {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        if (balance < totalCost) throw new Error('Wallet balance যথেষ্ট নয়। আগে Add Money করুন')

        tx.update(userRef, {
          walletBalance: balance - totalCost,
          matchesPlayed: (userSnap.data().matchesPlayed || 0) + 1,
        })
